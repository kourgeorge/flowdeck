...
def delete_user(db: Session, user_id: int) -> bool:
    ...
    execution_ids = [
        execution_id
        for (execution_id,) in db.query(Execution.id)
    ...
    try:
        if chat_session_ids:
            db.query(ChatTurn).filter(
                or_(
                    ...
                    ChatTurn.session_id.in_(chat_session_ids),
                )
            ...
            db.query(ChatTurn).filter(ChatTurn.user_id == user_id).delete(
        ...
        if execution_ids:
            ...
            db.query(Report).filter(
                Report.execution_id.in_(execution_ids)
            ).delete(synchronize_session=False)
            db.query(Execution).filter(Execution.id.in_(execution_ids)).delete(
                synchronize_session=False
            )
        else:
            db.query(ReportView).filter(ReportView.viewer_id == user_id).delete(
                synchronize_session=False
            )
