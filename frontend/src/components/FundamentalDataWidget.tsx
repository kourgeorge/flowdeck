...
interface FundamentalDataWidgetProps {
  data: FundamentalData | string;
}

const FundamentalDataWidget: React.FC<FundamentalDataWidgetProps> = ({ data }) => {
  // Parse data if it's a string (try JSON first, then fallback to plain text)
  ...
  // If we have structured data, display it in widgets
  if (fundamentalData) {
  ...
  // Fallback to text display if data is not structured
  if (textData) {
    return (
      <div className="bg-gray-800 rounded-lg border border-gray-700 p-6">
