...
export default function StockDetailPanel({ ticker, prefetchedData, onSubscriptionChange }: StockDetailPanelProps) {
  ...
  const activeValuationReport: ReportData | null = activeReportsSource?.valuation_report ?? null;
  const valuationSummary = activeValuationReport?.valuation_summary ?? null;
  const valuationRows = valuationSummary?.rows ?? [];
  const valuationWeightedAvg = valuationSummary?.weighted_avg ?? null;
  const valuationMethodWeights = valuationSummary?.method_weights ?? null;
        ...
        <div className="p-4 sm:p-6 space-y-4">
          ...
          {activeTab === 'overview' && (
            <div className="space-y-4">
              ...
              {activeValuationReport && quote && (
                <div className="bg-gray-800 rounded-lg border border-gray-700 p-4 sm:p-5">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                      <h3 className="text-sm sm:text-base font-semibold text-white">Valuation Snapshot</h3>
                      <p className="mt-1 text-xs sm:text-sm text-gray-400">
                        Latest valuation output from the AI valuation report for this run.
                      </p>
                    </div>
                    {activeValuationReport.valuation_conviction && (
                      <span className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold border ${
                        activeValuationReport.valuation_conviction === 'high'
                          ? 'border-green-700 bg-green-900/30 text-green-300'
                          : activeValuationReport.valuation_conviction === 'medium'
                            ? 'border-yellow-700 bg-yellow-900/30 text-yellow-300'
                            : 'border-red-700 bg-red-900/30 text-red-300'
                      }`}>
                        Conviction: {activeValuationReport.valuation_conviction}
                      </span>
                    )}
                  </div>
                  ...
                  <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
                    ...
                    <div className="rounded-lg border border-gray-700 bg-gray-900/40 p-3">
                      <div className="text-[11px] uppercase tracking-wide text-gray-400">Fair Value</div>
                      <div className="mt-1 text-lg font-semibold text-white">
                        {activeValuationReport.fair_value_base != null
                          ? formatPrice(activeValuationReport.fair_value_base, quote.currency)
                          : 'N/A'}
                      </div>
                    </div>
                    <div className="rounded-lg border border-gray-700 bg-gray-900/40 p-3">
                      <div className="text-[11px] uppercase tracking-wide text-gray-400">Discount / Premium</div>
                      <div className={`mt-1 text-lg font-semibold ${
                        (activeValuationReport.current_discount_pct ?? 0) >= 0 ? 'text-green-400' : 'text-red-400'
                      }`}>
                        {activeValuationReport.current_discount_pct != null
                          ? `${activeValuationReport.current_discount_pct >= 0 ? '+' : ''}${activeValuationReport.current_discount_pct.toFixed(2)}%`
                          : 'N/A'}
                      </div>
                    </div>
                    <div className="rounded-lg border border-gray-700 bg-gray-900/40 p-3">
                      <div className="text-[11px] uppercase tracking-wide text-gray-400">Valuation Score</div>
                      <div className="mt-1 text-lg font-semibold text-white">
                        {activeValuationReport.score != null ? `${activeValuationReport.score}/10` : 'N/A'}
                      </div>
                    </div>
                  </div>
                      ...
                      <table className="min-w-full text-left text-xs sm:text-sm">
                        <thead className="bg-gray-900/70">
                          <tr className="border-b border-gray-700">
                            <th className="px-3 py-2 font-medium text-gray-300">Method</th>
                            <th className="px-3 py-2 font-medium text-gray-300">Weight</th>
                            <th className="px-3 py-2 font-medium text-gray-300">Base</th>
                            <th className="px-3 py-2 font-medium text-gray-300">Implied</th>
                          </tr>
                        </thead>
                        <tbody>
                          {valuationRows.map((row: NonNullable<NonNullable<ReportData['valuation_summary']>['rows']>[number]) => (
                            <tr key={row.method} className="border-b border-gray-700/60 last:border-0">
                              <td className="px-3 py-2 text-white">{row.method}</td>
                              <td className="px-3 py-2 text-gray-300">{(row.weight * 100).toFixed(1)}%</td>
                              <td className="px-3 py-2 text-gray-300">{formatPrice(row.base, quote.currency)}</td>
                              <td className="px-3 py-2 text-gray-300">{formatPrice(row.implied_value, quote.currency)}</td>
                            </tr>
                          ))}
                          {valuationWeightedAvg && (
                            <tr className="bg-gray-900/60">
                              <td className="px-3 py-2 font-semibold text-white">Weighted Avg</td>
                              <td className="px-3 py-2 font-semibold text-gray-200">
                                {typeof valuationWeightedAvg.weight === 'number'
                                  ? `${(valuationWeightedAvg.weight * 100).toFixed(0)}%`
                                  : '100%'}
                              </td>
                              <td className="px-3 py-2 font-semibold text-gray-200">
                                {formatPrice(valuationWeightedAvg.base, quote.currency)}
                              </td>
                              <td className="px-3 py-2 font-semibold text-gray-200">
                                {formatPrice(valuationWeightedAvg.implied_value, quote.currency)}
                              </td>
                            </tr>
                          )}
                        </tbody>
                  ...
                  {valuationMethodWeights && Object.keys(valuationMethodWeights).length > 0 && (
                    <div className="mt-4 flex flex-wrap gap-2">
                      {Object.entries(valuationMethodWeights).map(([method, weight]: [string, number]) => (
                        <span
                          key={method}
                          className="inline-flex items-center rounded-full border border-blue-800/70 bg-blue-950/40 px-3 py-1 text-xs font-medium text-blue-200"
                        >
                          {method}: {(weight * 100).toFixed(1)}%
                        </span>
                      ))}
                    </div>
                  )}

                  {activeValuationReport.valuation_key_assumptions && activeValuationReport.valuation_key_assumptions.length > 0 && (
                    <div className="mt-4">
                      <div className="mb-2 text-xs font-semibold uppercase tracking-wide text-gray-400">
                        Key valuation assumptions
                      </div>
                      <ul className="space-y-1.5 text-sm text-gray-300">
                        {activeValuationReport.valuation_key_assumptions.slice(0, 4).map((assumption: string, index: number) => (
                          <li key={`${index}-${assumption}`} className="flex gap-2">
