...
└── tradingagents/   # Agents package (used by backend)
```
...
3. Run the backend from repo root so `tradingagents` is importable (or run from `backend/`; run.py adds repo root to PYTHONPATH).
...
**Important**: Run from the `backend` directory; `run.py` adds the repo root to PYTHONPATH so `tradingagents` can be imported.
