...
flowchart TB
    subgraph UI["Frontend (React/Vite)"]
        ...
        API_Client["api.ts → /api/data/* + /api/tickers/*"]
        Pages --> Components
...
| From | To | What |
|------|----|------|
| **UI** | Backend | Raw market data via `/api/data/*` (quote, news, fundamentals, statements, charts, historical, analyst recs). UI views via `/api/tickers/*` (widgets, full page). Start/status via `/api/analyses/*`. |
...
```
                    ...
                    └───────────────────────────┬─────────────────────────────┘
                                                 │ /api/data/*, /api/tickers/*, /api/analyses/*
                                                 ▼
┌──────────────┐    ┌─────────────────────────────────────────────────────────┐    ┌─────────────────┐
│   Yahoo      │◄───│              Backend (FastAPI)                            │───►│  results/       │
│  (yfinance)  │    │  • InfoFetcher (data) • /api/tickers/* (views)             │    │  <TICKER>/<DATE>│
