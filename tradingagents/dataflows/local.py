...
def get_reddit_global_news_online(
    ...
    try:
        ...
        return f"Error fetching Reddit news online: {str(e)}. Make sure REDDIT_CLIENT_ID, REDDIT_CLIENT_SECRET, and REDDIT_USER_AGENT are set."
...
def get_reddit_company_news_online(
    ...
    """
    try:
        # Build search query from ticker if available
        search_query = query
        if query in ticker_to_company:
            ...
            if "OR" in company_name:
                # Use the first part before OR
                search_query = company_name.split(" OR ")[0]
            else:
                search_query = company_name
            # Also include ticker in search
        ...
        posts = fetch_reddit_posts_online(
            ...
            end_date=end_date,
            limit=50,  # Fetch more to get better results
            sort="top",
            time_filter="month",
        )
